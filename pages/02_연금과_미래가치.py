아래는 **기말급·기시급 차이를 그래프로 보여주는 전체 코드**입니다.  
연도별 누적 미래가치를 계산해서 **두 선을 한 그래프에서 비교**하도록 넣었습니다.
...
### 이 버전에서 추가된 점
- **기말급과 기시급의 연도별 미래가치 비교 그래프**
- **최종 차이 금액 표시**
- **연도별 비교 표**
- 학생이 같은 조건에서 **왜 기시급이 더 큰지 시각적으로 확인 가능**

### 참고
현재 코드는 `rate` 최소값이 1.0이어서 괜찮지만,  
나중에 0%도 허용하려면 분모 `r` 때문에 예외처리가 필요합니다.

원하시면 다음 단계로
- **막대그래프 버전**
- **현재가치(PV)도 함께 비교하는 이중 그래프 버전**
- **학생용 설명 문구를 더 쉽게 바꾼 버전**
으로 바꿔드릴게요.